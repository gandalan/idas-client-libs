...
namespace Gandalan.IDAS.WebApi.Client.Discovery
{
    ...
    public class MailslotServer : IDisposable
    {
        ...
        public NetworkMessage GetNextMessage()
        {
            ...
            string serializedMessage = Encoding.Unicode.GetString(bBuffer);
            if (!string.IsNullOrEmpty(serializedMessage))
            {
                return JsonConvert.DeserializeObject<NetworkMessage>(serializedMessage);
            }
            return null;
        }
    ...
    public class MailslotClient : IDisposable
    {
        ...
        {
            NetworkMessage message = new NetworkMessage() { Content = messageContent, MessageGuid = Guid.NewGuid(), Sender = sender, Recipient = recipient };
            ...
            byte[] bMessage = Encoding.Unicode.GetBytes(serializedMessage);
